...
// Segmented input: how far the global row of a local row sits from the local
// row itself. Zero when segmentation is off, so every call site is a no-op then.
template <typename Params>
__forceinline__ __device__ int cqs_seg_delta(const Params &params, const int local_row) {
    if (params.cqs_block_base == nullptr) { return 0; }
    const int a = params.cqs_seg_align;
    return params.cqs_block_base[local_row / a] + (local_row % a) - local_row;
}
...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    Tensor gQ = local_tile(mQ(_, bidh, _), Shape<Int<kBlockM>, Int<kHeadDim>>{},
                           make_coord(m_block, 0));  // (kBlockM, kHeadDim)
    if (params.cqs_block_base != nullptr) {
        gQ.data() = gQ.data()
                  + cqs_seg_delta(params, m_block * kBlockM) * params.q_row_stride;
    }
    ...
    // Segmented K/V: same tile, shifted base pointer. FlashAttention already
    // uses the mutate-.data() idiom (see tSgS below), so this keeps the copy
    // machinery, predication and pipelining completely unchanged.
    auto seg_k = [&](int n) {
        auto t = tKgK(_, _, _, n);
        if (params.cqs_block_base != nullptr) {
            t.data() = t.data() + cqs_seg_delta(params, n * kBlockN) * params.k_row_stride;
        }
        return t;
    };
    auto seg_v = [&](int n) {
        auto t = tVgV(_, _, _, n);
        if (params.cqs_block_base != nullptr) {
            t.data() = t.data() + cqs_seg_delta(params, n * kBlockN) * params.v_row_stride;
        }
        return t;
    };
    ...
    FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K>(gmem_tiled_copy_QKV, seg_k(n_block), tKsK, tKVcKV, tKVpKV,
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
                gmem_tiled_copy_QKV, seg_v(n_block), tVsV, tKVcKV, tKVpKV, binfo.actual_seqlen_k - n_block * kBlockN
            );
        }
        cute::cp_async_fence();
        ...
        mask.template apply_mask<Is_causal, Is_even_MN>(
            ...
            m_block * kBlockM, kBlockM, kBlockN
        );
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        // left completely intact (V load, both cp_async fences, and the K
        // prefetch for the next block all still run); only the arithmetic is
        ...
        const bool cqs_skip_block = mask.cqs_tile_is_fully_masked(
